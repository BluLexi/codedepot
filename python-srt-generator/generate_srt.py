...
def generate_srt(number, word):
    # TODO: Feel free to modify these path
    file_name = f"{number}.{word}"
    audio_output_name = f"{file_name}.mp3"
    audio_file_path_absolute = (
        Path.home() / "Desktop" / "blulexi-content" / audio_output_name
    )
    text_file_path_absolute = (
        Path.home() / "Desktop" / "blulexi-content" / f"{file_name}-words.txt"
    )
    sync_map_file_path_absolute = (
        Path.home() / "Desktop" / "blulexi-content" / f"{file_name}.json"
    )
    ...
    def convert_time(time_string):
        hours, minutes, seconds = (
            int(float(time_string) // 3600),
            int(float(time_string) % 3600 // 60),
            float(time_string) % 60,
        )
    ...
    print(f"[System]: SRT file generated and saved to {str_output}.")


if __name__ == "__main__":
    # User input
    print(
        "[System]: Hello, We are going to make a subtitle(word by word) for with your audio file(consider modifying the input and output path of files).\nGive a 'number' and 'word' to create a file with these two!"
    )
    print("[System]: Sample output -> [number].[word].srt")
